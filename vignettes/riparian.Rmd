...
Welcome to the `vignette` for the package `riparian`, a suite of tools designed to help Benton County Community Development staff analyze riparian cover extent and change in cover over time.  This document models the workflow of producing an annual monitoring report for the Benton County riparian monitoring program, and demonstrates how functions in the `riparian` package can make monitoring faster and more efficient for the researcher.  The tools in this package adhere to the procedures described in the [Research Methods](methods.html) document.

## Table of Contents

 * [Step One: Site Plots](#site_plots)
 * [The Library Path](#library_path)
 * [Survey Boxes](#survey_boxes)
 * [Thumbnail Images](#thumbnails)
 * [Sample Plots](#sample_plots)
 * [Predicting Cover](#pred_cover)
 * [Scoring Samples](#scoring_samples)
 * [Subsetting Sites](#subsetting_sites)
 * [Plot Cover Extent](#plot_cover)
 * [Plot Cover Change](#plot_change)
 * [Clean Permit List](#clean_permits)
 * [Change Tables](#change_tables)
 * [Comparing Areas with Permits to Random Areas](#comparing_permits_to_random)
 * [Drawing New Survey Boxes](#sampler)
 * [Vetting Survey Boxes](#vetting)
   + [Process for Inspecting Boxes](#inspecting)
   + [Verifying Form and Position](#verifying_form)
   + [Selecting Lots with Active Permits](#selecting_active_permits)
   + [Overlapping Survey Boxes](#overlapping)
 * [Appending Samples](#append_samples)
 * [Exporting Survey Boxes](#export_boxes)
 * [Predicted Cover Tools](#predicting_cover)
   + [Predicting Cover Change](#predicting_change)
...
<br>
**[Example Sample Plot](examples/sample_22_2020.png)**
</center> 

The `plot_samples` function prints a .png file into the `out_path` directory for each survey box in the `polys` object, showing the survey box superimposed over the aerial imagery, with slices labeled by number (as shown in the example plot above).

<a name="scoring_samples"></a>
...
<br>
**[my_cover_plot.png](my_cover_plot.png)**
</center>

<a name="plot_change"></a>
...
<br>
**[my_change_plot.png](my_change_plot.png)**
</center>

<a name="clean_permits"></a>
 ...
 * If no discernible loss has occurred, but changes in camera aspect cause the position of tree canopy relative to the survey box to move, record as measurement error and disregard.
...
<a name="comparing_permits_to_random"></a>
...
# into a mean cover extent per survey box
...
## Vetting Survey Boxes

There are a chain of assumptions that occur when generating survey boxes.  We expect:

 * The randomly-selected point will occur on a stream path.
 * The stream path will occur in one of the specified tax lots.
 * The survey box will extend across 150-ft of stream path, and 50-ft into the riparian zone.  
 * Survey boxes will have 50 slices per box, with divisions between slices to extend perpendicular from the stream path.
 * The survey box will follow the higher order stream (main stem) at tributary junctions.  

These assumptions are requirements that must hold for survey boxes to be valid.  The `riparian` package depends upon a suite of complex spatial and data libraries, and my imperfect understanding of how those libraries function and interact.  Hence, one should view with a reasonable amount of suspicion the survey boxes output by the `sample_streams()` function until having had the chance to vet each survey box to ascertain that it meets each basic assumption, or requirement, imposed upon valid survey boxes.

The `inspect_samples()` functions is a tool designed to address the difficulty of verifying whether new samples meet all the validation criteria.  The full syntax of the function is:

```{r, eval=F}
inspect_samples(
  samples = my_new_samples, # replace with the name of new samples to vet
  in_path = '/path_to_imagery/', # replace with your own path to raster imagery
  out_path = getwd(), # path to output results, defaults to working directory
  permits = permits_13to18, # replace with cleaned list of active permits
  lots = sf_lots  # lots within the riparian corridor, can remain as-is
)
...
### Process for Inspecting Boxes

When run, `inspect_samples()` will print a series of PNG files to `out_path`.  Each picture will show a survey box from `samples`, as well as any maptaxlots overlain by the survey box, and the adjacent priority riparian corridor.  The figure labeled [Inspection Plot](#inspection) shows an example from 2020.

<a name="inspection"></a>
<center>
<img src="examples/inspection_plot.png" width = "650">
<br>
**[Inspection Plot](examples/inspection_plot.png)**
</center>

For each survey box in samples, the function looks for maptaxlots lying under the box and prints them in a black outline over raster imagery.  The maptaxlot number prints in the center of each taxlot.  Blue numbers indicate the maptaxlot has active county permits, while red numbers indicate no active permits.  The riparian corridor is superimposed over the top of the imagery in translucent blue.  The subject survey box will appear as a clear wire frame within the riparian corridor, while other survey boxes in the list will be red in color. Use the inspection plots to ascertain:

 * If a survey box has the correct form around a valid position on the stream path.
 * If a survey box overlays a maptaxlot with an active county permit.
 * If a survey box overlaps another survey box in the sample set.

<a name="verifying_form"></a>

#### Verfiying Form and Position

The sampling tool has known bugs that cause survey boxes to fail to draw correctly.  I would offer an expected date when these bugs might be resolved, but let's just say I was lucky to get the sampler function working at all.  When you generate a series of survey boxes, it is possible that many will be good boxes and some will be [bad boxes](#bad_boxes).  A box that is properly drawn will extend 150-ft along the stream path, and extend 50-ft into the riparian corridor, divided into 25 evenly-spaces slices on either stream bank, with divisions between slices extending perpendicular from the direction of the stream path.  A box that is properly positioned is centered on the stream path within the priority riparian corridor, as delineated in blue on the inspection plots.

<a name="bad_boxes"></a>
<center>
<img src="examples/bad_boxes.png" width = "650">
<br>
**[Bad Boxes](examples/bad_boxes.png)**
</center>

<a name="selecting_active_permits"></a>

#### Selecting Lots With Active Permits

When sampling a subset of taxlots (lots with active permits), compared to the corridor as a whole (randomly-selected lots), the `sample_streams()` function can fail to properly restrict the sampling area to the subset of lots, and unintentionally
draw samples from unrelated parts of the priority corridor outside the study area of interest (lots without active permits).  This is a known issue that will hopefully be resolved in the near future.

The color of the maptaxlot number on the inspection plot indicates whether the maptaxlot is associated with an active permit.  Blue maptaxlot numbers indicate active permits, while red numbers indicate no active permits.  When sampling lots with active permits, the centerpoint of the survey box (the sampling point) should occur within a lot associated with an active permit, or a blue maptaxlot number.

<a name="overlapping"></a>

#### Overlapping Survey Boxes

Two survey boxes may be valid individually, but if they overlap one another then we cannot use both in the same sample set, because their cover scores are not independent of one another.  The subject survey box has a clear black outline, while additional survey boxes in the list have a red fill.  If a red survey box overlaps the clear black survey box, then there is a conflict with the subject sample, and we can only include one or the other in the analysis.  Standard practice is to retain valid survey boxes in the order produced and discard any subsequent samples that overlap.

To check if new samples (samples with permits) overlap with a pre-existing set of samples (random samples), [append the two sample sets together](#append_samples) before running `inspect_samples()`.  The pre-existing samples will appear as red survey boxes when new survey boxes are the subject of the plot, and you can disregard inspection plots where the pre-existing survey boxes are the subject.

My strategy to deal with invalid boxes generally was to create more survey boxes than I needed, and then to delete bad or invalid boxes from the resulting spatial object.  R recognizes the survey boxes as spreadsheets with associated spatial data.  Each survey box is a row of data, and you can delete unwanted rows as shown in the code snippet below.  Note the use of the concatenate function `c()` to delete multiple samples at once.
...
<a name="append_samples"></a>

### Appending Samples

When you have a pre-existing set of survey boxes, you may wish to append additional survey boxes to the end.  For instance, you may have a set of survey boxes used for permit-based monitoring in a previous report.  When presented with a list of new permits the county has issued since the last report, you may elect to take a smaller number of samples from the new pool and add these to the pre-existing set.

Since the survey boxes are represented as lists, you construct a new sampling object by concatenating two lists together using the `c()` function.
...
<a name="export_boxes"></a>

### Exporting Survey Boxes
...
<a name="predicting_cover"></a>

## Predicted Cover Tools
...
<a name="predicting_change"></a>

## Predicting Cover Change
